...
if sqlcipher_spec and sqlcipher_spec.submodule_search_locations:
    ...
    for root, dirs, files in os.walk(sqlcipher_dir):
        for file in files:
            ...
            if (
                file_lower.endswith((".dll", ".dylib", ".so", ".pyd"))
                or ".so." in file_lower
            ):
...
if platform.system().lower() == "windows" or sys.platform == "win32":
            ...
            for file in files:
                ...
                if file_lower.endswith(".dll"):
                    ...
                    if file_lower not in found_dll_names:
                        ...
                        print(
                            f"Bundling required Windows dependency DLL from sqlcipher3 package: {dll_path}"
                        )
    ...
    for s_dir in search_dirs:
        ...
        try:
            ...
            print(
                f"Warning: Could not list directory {s_dir} during DLL discovery: {scan_err}"
            )
            continue
        for file in files_in_dir:
            file_lower = file.lower()
            if file_lower.endswith(".dll") and any(
...
if not custom_sqlite3_dll:
    ...
    for vd in venv_dirs:
        prefix_lower = vd.lower().replace("\\", "/")
        base_lower = (
            sys.base_prefix.lower().replace("\\", "/") if sys.base_prefix else None
        )
        ...
        for sub in ["Library/bin", "Scripts", "Lib/site-packages/sqlcipher3"]:
            ...
            if os.path.isdir(candidate_dir):
                candidate_path = os.path.abspath(
                    os.path.join(candidate_dir, "sqlite3.dll")
                )
        ...
        for root, dirs, files in os.walk(vd):
            ...
            for file in files:
                if file.lower() == "sqlite3.dll":
                    ...
                    if (
                        base_lower
                        and base_lower in cand_lower
                        and base_lower != prefix_lower
                    ):
                        continue
                    custom_sqlite3_dll = candidate_path
                    break
...
for x in a.binaries:
    ...
    if dest_lower.endswith("sqlite3.dll") and custom_sqlite3_dll:
        if not (
            ...
            print(
                f"Redirecting standard sqlite3.dll dependency {src_path} -> custom {custom_sqlite3_dll}"
            )
...
a.datas = [
    x for x in a.datas if not is_tcl_tk_asset(x[0]) and not is_prunable_asset(x[0])
]
