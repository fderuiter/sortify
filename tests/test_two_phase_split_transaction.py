...
def test_fast_path_plan_isolation(temp_environment):
    ...
    docs = [
        (
            temp_environment["base_dir"],
            temp_environment["policy_file"],
            "hash1",
            "compliance report",
        ),
        (
            temp_environment["base_dir"],
            temp_environment["keyword_file"],
            "hash2",
            "invoice details",
        ),
        (
            temp_environment["base_dir"],
            temp_environment["ai_file"],
            "hash3",
            "unknown deep content",
        ),
    ]
    ...
    with mock.patch.object(
        session.analyzer.embedding_manager, "get_vectors_batch"
    ) as mock_get_vectors:
...
def test_ocr_bypassing_for_fast_path_moved_files(temp_environment):
    ...
    moved_path = os.path.join(
        temp_environment["base_dir"], "Invoices", temp_environment["keyword_file"]
    )
    ...
    items_to_sort_ph2 = [
        f for f in refreshed_files_norm if f not in bypassed_files_ph2_norm
    ]
...
def test_independent_rollback_safety(temp_environment):
    ...
    invoice_target_path = os.path.join(
        temp_environment["base_dir"], "Invoices", temp_environment["keyword_file"]
    )
    assert os.path.exists(invoice_target_path)
    assert not os.path.exists(
        os.path.join(temp_environment["base_dir"], temp_environment["keyword_file"])
    )
    ...
    with mock.patch(
        "app.core.mover._execute_moves_recursive",
        side_effect=OSError("Disk failure during AI phase moves"),
    ):
    ...
    assert os.path.exists(
        os.path.join(temp_environment["base_dir"], temp_environment["ai_file"])
    )
