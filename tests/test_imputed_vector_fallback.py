...
def test_imputed_vector_fallback_partial_missing():
    ...
    with patch.object(TfidfVectorizer, "__init__", mock_init):
        ...
        assert 1000 not in init_max_features, (
            f"Should not fall back to TF-IDF (max_features=1000) when some valid vectors exist! Max features logged: {init_max_features}"
        )
...
def test_imputed_vector_fallback_100_percent_missing():
    ...
    with patch.object(TfidfVectorizer, "__init__", mock_init):
        ...
        assert 1000 in init_max_features, (
            "Must fall back to TF-IDF (max_features=1000) when 100% of the embeddings are missing!"
        )
        assert plan is not None
...
def test_no_alignment_exceptions_with_different_dimensions():
    ...
    try:
        ...
        pytest.fail(
            f"Mathematical alignment or dimension mismatch exception occurred: {e}"
        )
