...
from app.ui.app import AutoSorterApp, find_and_remove_file, insert_file_into_plan
...
def test_render_tree_uses_in_memory_cache(test_env):
    ...
    with patch.object(
        db, "get_all_document_ratings", side_effect=AssertionError("Should not query DB during render_tree")
    ):
        app.render_tree()
...
def test_handle_node_drop_instantly_updates_cache(test_env):
    ...
    with patch.object(db, "set_user_verified_target_path", wraps=db.set_user_verified_target_path) as mock_write:
...
def test_handle_node_rate_instantly_updates_cache(test_env):
    ...
    with patch.object(db, "set_document_rating", wraps=db.set_document_rating) as mock_write:
