...
def test_non_windows_platform_guardrail():
    ...
    with patch("sys.platform", "linux"):
        ...
        assert "Context menu integration is only available on Windows." in str(excinfo.value)
...
def test_windows_admin_escalation_failure(mock_winreg_and_ctypes):
    ...
    _, mock_ctypes = mock_winreg_and_ctypes
    mock_ctypes.windll.shell32.IsUserAnAdmin.return_value = False
    mock_ctypes.windll.shell32.ShellExecuteW.return_value = 5  # Failure return code <= 32
...
def test_windows_admin_registry_operations_enable_packaged(mock_winreg_and_ctypes):
    ...
    with patch("sys.platform", "win32"), \
        ...
        mock_winreg.CreateKey.assert_any_call("HKEY_CLASSES_ROOT", r"Directory\shell\SmartAutoSorter")
        mock_winreg.CreateKey.assert_any_call(mock_key_dir, "command")
        mock_winreg.CreateKey.assert_any_call("HKEY_CLASSES_ROOT", r"Directory\Background\shell\SmartAutoSorter")
        ...
        mock_winreg.SetValue.assert_any_call(mock_key_dir, "", 1, "Open in Smart Auto-Sorter")
        mock_winreg.SetValue.assert_any_call(mock_key_bg, "", 1, "Open in Smart Auto-Sorter")
...
def test_windows_admin_registry_operations_enable_unpackaged(mock_winreg_and_ctypes):
    ...
    with patch("sys.platform", "win32"), \
        ...
        commands_set = [args[3] for args in set_value_args if "%1" in args[3] or "%V" in args[3]]
...
def test_windows_admin_registry_operations_disable(mock_winreg_and_ctypes):
    ...
    with patch("sys.platform", "win32"):
        ...
        mock_winreg.DeleteKey.assert_any_call("HKEY_CLASSES_ROOT", r"Directory\shell\SmartAutoSorter\command")
        mock_winreg.DeleteKey.assert_any_call("HKEY_CLASSES_ROOT", r"Directory\shell\SmartAutoSorter")
        mock_winreg.DeleteKey.assert_any_call("HKEY_CLASSES_ROOT", r"Directory\Background\shell\SmartAutoSorter\command")
        mock_winreg.DeleteKey.assert_any_call("HKEY_CLASSES_ROOT", r"Directory\Background\shell\SmartAutoSorter")
...
def test_settings_toggle_on_explorer_integration_windows_success(mock_winreg_and_ctypes):
...
def test_settings_toggle_on_explorer_integration_windows_failure(mock_winreg_and_ctypes):
    ...
    with patch("app.ui.settings.ui") as mock_ui, \
         patch("sys.platform", "win32"), \
         patch("app.core.integration.register_context_menu", side_effect=RuntimeError("Elevation refused")):
...
def test_main_cli_directory_argument():
    ...
    with patch("app.main.argparse.ArgumentParser.parse_args", return_value=mock_args), \
         patch("app.ui.app.run_app") as mock_run_app, \
         patch("app.main.AppSettings") as mock_settings_class:
         
        main()
        
        mock_run_app.assert_called_once_with(mock_settings_class.return_value, "/some/test/directory")
