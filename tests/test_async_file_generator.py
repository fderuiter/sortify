...
async def test_build_corpus_generator_async_cache_skip(tmp_path):
    ...
    db.upsert_document(str(base_dir), "file1.txt", fhash, "This is extracted cached text!")
...
async def test_scan_and_process_worker_ui_updates(tmp_path):
    ...
    with patch("app.core.scanner.get_files_recursively", return_value=files), \
