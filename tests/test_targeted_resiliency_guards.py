...
from app.core.mover import get_safe_path, _remove_empty_dirs, execute_moves
...
def test_collision_loop_limit(tmp_path):
    ...
    with patch("os.path.lexists", return_value=True):
        ...
        assert "Unique name cannot be resolved within 1,000 attempts" in str(exc_info.value)
...
def test_transactional_sync_execution_on_failure(setup_mover_env):
    ...
    with conn:
        conn.execute(
            ...
            (str(base_dir), "test.txt", "hash123", db.crypto.encrypt_text("extracted_text"))
        )
    ...
    with patch("app.core.resilient_file_ops.resilient_move", side_effect=OSError("Write failure")):
...
def test_active_rollback_bypasses_batch_updates(setup_mover_env):
    ...
    updates = [
        {
            "type": "document_path",
            "args": (str(base_dir), "test.txt", "dest.txt")
        }
    ]
