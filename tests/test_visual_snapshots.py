...
"""
    ...
    launcher_file = tempfile.NamedTemporaryFile(
        suffix=".py", mode="w", delete=False, dir=TESTS_DIR
    )
    ...
    env["AUTOSORTER_SESSION_BASE_DIR"] = os.path.join(
        isolated_tmp, "autosorter_sessions"
    )
    ...
    with sync_playwright() as p:
        ...
        try:
            page.add_init_script(
                ...
                f"style.innerHTML = `{disable_animations_css}`; "
                f"(document.head || document.documentElement).appendChild(style);"
            )
        except Exception:
            pass
        ...
        decline_btn = page.locator('.q-dialog [aria-label="Decline Button"]').first
        try:
            if decline_btn.is_visible():
                decline_btn.click()
            else:
                menu_btn = page.locator(".q-dialog .overflow-toolbar-menu-btn").first
                if menu_btn.is_visible():
                    menu_btn.click()
                    page.wait_for_timeout(300)
                    page.locator(
                        '.q-menu [aria-label="Decline Button"], .q-menu:has-text("Decline")'
                    ).first.click()
                else:
                    page.evaluate(
                        "() => { const btns = Array.from(document.querySelectorAll('button')); const b = btns.find(el => el.getAttribute('aria-label') === 'Decline Button' || el.textContent.includes('Decline')); if (b) b.click(); }"
                    )
        except Exception:
            page.evaluate(
                "() => { const btns = Array.from(document.querySelectorAll('button')); const b = btns.find(el => el.getAttribute('aria-label') === 'Decline Button' || el.textContent.includes('Decline')); if (b) b.click(); }"
        ...
        settings_btn = page.locator('[aria-label="Settings Button"]').first
        if not settings_btn.is_visible():
            page.wait_for_selector('[aria-label="More Actions Menu"]', timeout=5000)
        else:
        ...
        page.wait_for_timeout(
            2000
        )  # wait for animations and fade transitions to settle
        ...
        try:
            if settings_btn.is_visible():
                ...
                header_menu = page.locator('[aria-label="More Actions Menu"]').first
                if header_menu.is_visible():
                    header_menu.click()
                    page.wait_for_timeout(500)
                    page.locator(
                        '.q-menu [aria-label="Settings Button"], .q-menu .q-item:has-text("Settings")'
                    ).first.click()
                else:
                    page.evaluate(
                        "() => { const items = Array.from(document.querySelectorAll('button, .q-item, [aria-label=\"Settings Button\"]')); const el = items.find(x => x.getAttribute('aria-label') === 'Settings Button' || (x.textContent && x.textContent.includes('Settings'))); if (el) el.click(); }"
                    )
        except Exception:
            page.evaluate(
                "() => { const items = Array.from(document.querySelectorAll('button, .q-item, [aria-label=\"Settings Button\"]')); const el = items.find(x => x.getAttribute('aria-label') === 'Settings Button' || (x.textContent && x.textContent.includes('Settings'))); if (el) el.click(); }"
...
def test_visual_snapshots(nicegui_server, viewport_name, width, height):
    run_visual_snapshot_pass(
        nicegui_server, viewport_name, width, height, pseudoloc=False
    )


@pytest.mark.skipif(not PLAYWRIGHT_AVAILABLE, reason="playwright is not installed")
@pytest.mark.parametrize("viewport_name,width,height", VIEWPORTS)
def test_visual_snapshots_pseudoloc(
    nicegui_server_pseudoloc, viewport_name, width, height
):
    run_visual_snapshot_pass(
