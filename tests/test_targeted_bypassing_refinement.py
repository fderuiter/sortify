...
def test_targeted_bypassing_refinement_acceptance_criteria(tmp_path):
    ...
    try:
        ...
        bypassed = MetadataPass.run(
            str(base_dir), [comp_file_name, normal_file_name], MockSettings, db, None, None
        )
        sync_db()
        ...
        plan = analyzer.generate_sorting_plan(str(base_dir), runtime_settings=MockSettings())
        ...
        bypassed_new = MetadataPass.run(
            str(base_dir), [comp_file_name, normal_file_name], MockSettingsNew, db, None, None
        )
        sync_db()
        assert comp_file_name in bypassed_new
        
        # Re-run analyzer with new settings
        plan_new = analyzer.generate_sorting_plan(str(base_dir), runtime_settings=MockSettingsNew())
        ...
        assert "Old Archive" not in plan_new or comp_file_name not in plan_new.get("Old Archive", {})
        ...
        db.set_user_verified_target_path(str(base_dir), comp_file_name, "User Dragged Folder")
        sync_db()
        ...
        plan_after_drag = analyzer.generate_sorting_plan(str(base_dir), runtime_settings=MockSettingsNew())
        assert "User Dragged Folder" in plan_after_drag
        assert comp_file_name in plan_after_drag["User Dragged Folder"]
        assert plan_after_drag["User Dragged Folder"][comp_file_name].get("is_conflicted") is True
        ...
        bypassed_none = MetadataPass.run(
            str(base_dir), [comp_file_name, normal_file_name], MockSettingsNoRules, db, None, None
        )
        sync_db()
        plan_no_rules = analyzer.generate_sorting_plan(str(base_dir), runtime_settings=MockSettingsNoRules())
