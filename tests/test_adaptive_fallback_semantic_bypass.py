...
def test_mock_state_detection(db, temp_dir):
    ...
    manager_non_existent = SemanticEmbeddingManager(db, model_path=str(non_existent_path))
...
def test_statistical_fallback_on_mock(db, temp_dir):
    ...
    with patch.object(analyzer.embedding_manager, 'generate_embedding', wraps=analyzer.embedding_manager.generate_embedding) as mock_gen, \
         patch.object(analyzer.embedding_manager, 'trigger_reconstruction', wraps=analyzer.embedding_manager.trigger_reconstruction) as mock_reconstruct:
...
def test_dynamic_transition_to_semantic(db, temp_dir):
    ...
    with patch("app.core.semantic_embeddings.get_active_model_properties", return_value=valid_properties), \
         patch.object(analyzer.embedding_manager, "generate_embedding", side_effect=mock_gen_embedding) as mock_gen:
        ...
        db.upsert_document_vectors(base_dir, [("historical_space.txt", dummy_vector_hist)])
