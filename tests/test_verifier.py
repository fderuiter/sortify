...
class VirtualFS:
    def __init__(self, os_system="Linux", base_dir=None):
        ...
        self.base_dir = base_dir if base_dir is not None else ("C:\\app" if os_system == "Windows" else "/app")
...
def run_with_vfs(vfs: VirtualFS, func, *args, **kwargs):
    with patch("app.core.verifier.os.path.abspath", side_effect=vfs.abspath), \
         ...
         patch("app.core.verifier.os.path.basename", side_effect=vfs.path_module.basename), \
         ...
         patch("app.core.verifier.os.path.splitdrive", side_effect=vfs.path_module.splitdrive), \
...
def test_windows_path_limit():
    ...
    long_dest = "a" * 255
    plan = {
        "test.txt": {"__type__": "file", "target_filename": long_dest}
    }
...
def test_symlink_privilege_blocked(mock_get_link_info):
    ...
    assert errors["symlink.txt"] == "Operating system blocks link modification due to permission constraints"
...
def test_insufficient_space_different_file():
    ...
    plan = {
        "/mnt/backup": {"big.txt": None}, # this causes insufficient space on /mnt/backup
        "/": {"other.txt": None} # this is moving from backup to /, freeing space on backup, but let's just make it a move
    }
...
def test_windows_normal_path():
    ...
    plan = {
        "test.txt": {"__type__": "file", "target_filename": "normal.txt"}
    }
