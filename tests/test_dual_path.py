    ...
    with patch("app.core.session.get_app_dir", return_value=Path(app_temp)):
        ...
        with patch.object(sys, "frozen", False, create=True):
            with patch.object(
                ...
                with patch(
                    "app.core.session.os.path.abspath",
                    return_value=os.path.join(base_temp, "app", "core", "session.py"),
                ):
...
def test_strategy_dual_path_resolution_local_priority(mock_app_session_env):
    ...
    with patch("app.config.get_app_dir", return_value=Path(app_temp)):
        ...
        with patch.object(sys, "frozen", False, create=True):
            with patch(
                "app.core.analyzer_strategies.os.path.abspath",
                return_value=os.path.join(
                    base_temp, "app", "core", "analyzer_strategies.py"
                ),
            ):
...
def test_setup_wizard_bypass_dual_path(mock_app_session_env):
    ...
    with patch("app.config.get_app_dir", return_value=Path(app_temp)):
        ...
        with patch.object(sys, "frozen", False, create=True):
            with patch(
                "app.ui.app.os.path.abspath",
                return_value=os.path.join(base_temp, "app", "ui", "app.py"),
            ):
