...
- `actions/checkout` (via `.github/workflows/ci.yml`): Fetching source code in CI.
- `actions/checkout` (via `.github/workflows/release.yml`): Fetching source code in CI.
- `all-MiniLM-L6-v2` (via `app/ui/settings.py`): Runtime model download prompt in the UI wizard.
...
- `astral-sh/setup-uv` (via `.github/workflows/ci.yml`): Setting up uv in CI.
- `astral-sh/setup-uv` (via `.github/workflows/release.yml`): Setting up uv in CI.
