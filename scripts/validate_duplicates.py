...
class DuplicatePatternVisitor(ast.NodeVisitor):
    """AST visitor to find duplicate/redundant system utilities or path validation logic."""
    ...
    def visit_Attribute(self, node):
        """Detect direct access to sys.frozen."""
    ...
    def visit_Call(self, node):
        """Detect calls to getattr(sys, 'frozen')."""
        if self.filepath not in ALLOWED_FOR_FROZEN:
    ...
    def visit_Constant(self, node):
        """Detect illegal hardcoded string references/patterns."""
...
def main():
    """Run duplicate and redundant pattern checks on python sources."""
    errors = []
