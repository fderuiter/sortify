...
class HistoryManager:
    ...
    def _unwind_session_internal(self, session_id: str, db=None) -> int:
        ...
        with conn:
            ...
            snapshot_files = {r[0] for r in cur.fetchall()}
        ...
        if snapshot_files and base_dir and os.path.exists(base_dir):
            for root, dirs, files in os.walk(base_dir):
                for f in files:
                    ...
                    if rel_path not in snapshot_files:
                        try:
                            target_dirs_to_clean.add(os.path.dirname(full_path))
                            os.remove(full_path)
                        except Exception:
                            pass
