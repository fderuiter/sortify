...
def register_context_menu(enable: bool):
    ...
    if not is_admin():
        ...
        if int(ret) <= 32:
            raise RuntimeError(f"Failed to elevate privileges. ShellExecuteW returned {ret}")
        return True
    ...
    try:
        ...
        _set_keys(winreg.HKEY_CLASSES_ROOT, r"Directory\shell\SmartAutoSorter", enable, command_str)
        
        # Register for directory background (Right clicking inside an empty space in a folder)
        _set_keys(winreg.HKEY_CLASSES_ROOT, r"Directory\Background\shell\SmartAutoSorter", enable, bg_command_str)
    except Exception as e:
