...
class IncrementalAnalyzer:
    ...
    def reload_stop_words(self, new_stop_words: set) -> None:
        """Reload stop words from config (no-op for dense vectors, but updates stop words for keywords)."""
            ...
            def _annotate(node, current_path):
                for k, v in list(node.items()):
                    if v is None:
                        ...
                        contextual_renaming = (
                            settings.CONTEXTUAL_RENAMING if settings else False
                        )
    ...
    def _get_cluster_keywords(self, documents: list) -> str:
        ...
        try:
            vectorizer = TfidfVectorizer(
                stop_words=list(self.stop_words), max_features=3
            )
