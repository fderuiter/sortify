...
class AutoSorterApp:
    ...
    def resume_session(self, session_info):
        ...
        try:
            ...
            if "key file is missing" in str(e) or "key is missing" in str(e):
                import tempfile
                from pathlib import Path

                from app.ui.recovery import show_recovery_screen
                ...
                show_recovery_screen(
                    self, session_info["session_id"], session_info, "resume", db_path
                )
                return
            else:
                raise
    ...
    def revert_session(self, session_info):
        ...
        try:
            ...
            if "key file is missing" in str(e) or "key is missing" in str(e):
                ...
                show_recovery_screen(
                    self, session_info["session_id"], session_info, "revert", db_path
                )
                return
            else:
                raise
    ...
    def start_analysis(self):
        ...
        try:
            ...
            if "key file is missing" in str(e) or "key is missing" in str(e):
                ...
                show_recovery_screen(self, "", None, "analysis", db_path)
                return
            else:
                raise

        # Check if a new recovery code was generated!
        if (
            self.app_session
            and hasattr(self.app_session, "db")
            and hasattr(self.app_session.db, "crypto")
        ):
            rec_code = getattr(self.app_session.db.crypto, "recovery_code", None)
            if rec_code:
                from app.ui.recovery import show_recovery_code_onboarding

                show_recovery_code_onboarding(self, rec_code)
